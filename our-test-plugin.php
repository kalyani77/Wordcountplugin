...
function addTOEndOfPost($content){
	...
	return $content."<p>Hi, End Of Post</p>";
	}
}
*/
...
class WordCountAndTimePlugin{
	function __construct(){
		add_action('admin_menu', array($this, 'adminPage'));  //Add option of plugin in wordpress dashboard
		add_action('admin_init', array($this, 'settings'));         // setting of wordcount plugin
		add_filter('the_content',array($this, 'Ifwrap'));      //Adding Word count, Read Time show on Post 
		add_action('init',array($this,'languages'));            //langauges for plugin
	}
	...
	function createHTML($content){
		...
		if(get_option('wcp_charcount','1')){
			$html.='this post has '.strlen(strip_tags($content)).' character<br/>';
			
		}
		
		if(get_option('wcp_readtime','1')){
			$html.='this postwill take about '.round($wordCount/225).' minutes(s) to read.<br/>';
 ...
 <?php }
   ...
   <?Php
	...
	function adminPage(){
     add_options_page( 'Word Count Settings', __('Word Count','wcpdomain'), 'manage_options', 'Word_count_settings_page', array($this, 'OurHTML'));
